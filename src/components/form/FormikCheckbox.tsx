...
const FormikCheckbox = ({ label, errorMessage, ...props }: { label?: string; errorMessage?: string } & FieldAttributes<any>) => (
  <label className="inline-flex items-center gap-2.5 cursor-pointer group min-h-[40px]">
    <Field
      {...props}
      type="checkbox"
      className="w-4 h-4 sm:w-[18px] sm:h-[18px] text-gray-900 border-gray-300 rounded focus:ring-gray-400 focus:ring-2 cursor-pointer"
    />
    {label && (
