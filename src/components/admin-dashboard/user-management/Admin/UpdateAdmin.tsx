...
const EditAdmin = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      const id = Array.isArray(adminId) ? adminId[0] : adminId;
      if (!id) return;
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const url = API_URL.UPDATE_ADMIN.replace(':adminId', adminId as string);
  ...
  if (loading) {
    return <div className="text-center py-10 text-lg font-medium">Loading admin data...</div>;
  }

  return (
    <div className="container mx-auto min-h-screen flex flex-col items-center justify-center px-4 md:px-0 py-8">
      <h2 className="text-[24px] font-[500] text-[#1C1C1C] text-center mb-6">Edit Admin</h2>
      <Formik
